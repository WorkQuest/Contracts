task("config_workquest", "Config workquest")
    .setAction(async function (args, hre, runSuper) {
        ...
        if (!process.env.WORKQUEST_FEE_EMPLOYER) {
            throw new Error(
                `Please set your WORKQUEST_FEE_EMPLOYER in a .env-${network} file`
            )
        }
        if (!process.env.WORKQUEST_FEE_WORKER) {
            throw new Error(
                `Please set your WORKQUEST_FEE_WORKER in a .env-${network} file`
            )
        }
        if (!process.env.WORKQUEST_FEE_TX) {
            throw new Error(
                `Please set your WORKQUEST_FEE_TX in a .env-${network} file`
            )
        }
        if (!process.env.WORKQUEST_FEE_RECEIVER) {
            throw new Error(
                `Please set your WORKQUEST_FEE_RECEIVER in a .env-${network} file`
            )
        }
        if (!process.env.PENSION_FUND) {
            throw new Error(
                `Please set your PENSION_FUND in a .env-${network} file`
            )
        }
        if (!process.env.REFERRAL) {
            throw new Error(
                `Please set your REFERRAL in a .env-${network} file`
            )
        }
        if (!process.env.WUSD_TOKEN) {
            throw new Error(
                `Please set your WUSD_TOKEN in a .env-${network} file`
            )
        }
        ...
        const tx = await factory.referral()
        console.log('Done.', tx)
    });